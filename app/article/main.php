...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		{
			TPL::assign('redirect_info', $this->model('post')->get_post_by_id('article', $thread_info['redirect_id']));
		}
		if (H::GET('rf'))
		{
			TPL::assign('redirected_from', $this->model('post')->get_post_by_id('article', H::GET('rf')));
		}
		...
		// 判断是否已合并
		if ($redirect_posts = $this->model('post')->get_redirect_threads('article', $thread_info['id']))
		{
		...
		$this->model('post')->update_view_count('article', $thread_info['id']);
		...
		TPL::assign('related_posts', $this->model('threadindex')->get_related_posts_by_topic_ids('article', $topic_ids, $thread_info['id']));
		TPL::assign('recommended_posts', $this->model('threadindex')->get_recommended_posts('article', $thread_info['id']));
