...
class ajax extends AWS_CONTROLLER
{
	...
	public function edit_topic_action()
	{
		...
		if (!($this->user_info['permission']['manage_topic']))
		{
			...
			{
				H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('锁定的话题不能编辑')));
			}
		}
	...
	public function save_related_topic_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('请输入话题标题')));
		}
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('话题已锁定或没有创建话题权限')));
		}

		if (!$this->model('topic')->save_related_topic(H::GET('topic_id'), $related_id))
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('已经存在相同推荐话题')));
		}
	...
	public function merge_topic_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('话题合并不能与自己合并')));
		}
		...
		if ($topic_info['merged_id'])
		{
			...
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('该话题已经与 %s 合并', $merged_topic_info['topic_title'])));
		}
	...
	public function remove_topic_relation_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('指定的项目不存在')));
		}
		...
		if ($thread_info['lock'])
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('锁定的主题不能编辑话题')));
		}
	...
	public function save_topic_relation_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', _t('话题已锁定或没有创建话题权限, 不能添加话题')));
		}
