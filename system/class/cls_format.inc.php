...
class FORMAT
{
	public static function parse_image($url)
	{
		...
		return '<a href="url/img/' . safe_urlencode(htmlspecialchars_decode($url)) . '" target="_blank">' . 
	...
	public static function parse_link($url, $title = null)
	{
		...
		return '<a href="url/link/' . safe_urlencode(htmlspecialchars_decode($url)) . '" target="_blank">' . $title . '</a>';
	}
